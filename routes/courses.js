...
router.post('/course/add', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(PROF_ADD_STUDENT_TO_COURSE, [data.suname, data.module_code], (err, dbRes) => {
                    if (err) {
                        res.send("error!")
                    } else {
...
router.post('/course/:module_code/assessment/add', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(ADD_NEW_COURSE_ASSESSMENT, [data.module_code, data.title, data.max_mark], (err, dbRes) => {
                    if (err) {
                        res.send("error!")
                    } else {
...
router.post('/course/:module_code/assessment/delete', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(DELETE_COURSE_ASSESSMENT, [data.module_code, data.title], (err, dbRes) => {
                    if (err) {
                        res.send("error!")
                    } else {
...
router.post('/course/:module_code/assessment/edit', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(EDIT_COURSE_ASSESSMENT, [data.new_title, data.max_mark, data.module_code, data.old_title], (err, dbRes) => {
                    if (err) {
                        console.log(err)
                        res.send("error!")
                    } else {
...
router.post('/course/:module_code/gradebook', (req, res, next) => {
    ...
    pool.query(GET_STUDENT_COURSE_SCORES, [data.module_code, data.suname], (err, dbRes) => {
        if (err) {
            res.send('error!')
        } else {
            res.send(dbRes.rows)
...
router.post('/course/:module_code/gradebook/add', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(ADD_NEW_COURSE_ASSESSMENT_SCORE, [data.suname, data.module_code, data.score, data.title], (err, dbRes) => {
                    if (err) {
                        res.send("error!")
                    } else {
...
router.delete('/course/:module_code/gradebook/delete', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(DELETE_COURSE_ASSESSMENT_SCORE, [data.suname, data.module_code, data.title], (err, dbRes) => {
                    if (err) {
                        res.send("error!")
                    } else {
...
router.put('/course/:module_code/gradebook/edit', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(EDIT_COURSE_ASSESSMENT_SCORE, [data.score, data.suname, data.module_code, data.title], (err, dbRes) => {
                    if (err) {
                        res.send("error!")
                    } else {
...
router.post('/course/passed/add', (req, res, next) => {
    ...
    pool.query(CHECK_PROF_TEACHES, [data.module_code, data.puname], (err, dbRes) => {
        if (err) {
            ...
            if (dbRes.rowCount > 0) {
                pool.query(ADD_STUDENT_TO_PASSED, [data.suname, data.module_code, data.academic_year, data.semester, data.grade], (err, dbRes) => {
                    if (err) {
                        res.send("error!")
                    } else {
